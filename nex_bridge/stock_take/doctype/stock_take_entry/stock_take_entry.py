	...
	def _resolve_items_from_barcodes(self):
		...
		for row in self.items or []:
			...
			matches = frappe.get_all(
				"Item Barcode", filters={"barcode": row.barcode}, fields=["parent"], limit=2
			)
			...
			if len(matches) > 1:
				frappe.throw(
					_("Row #{0}: Multiple Items found for barcode {1}. Please set the Item Code.")
...
def create_stock_reconciliation(stock_take_entry: str, purpose: str):
	...
	if doc.docstatus != 1:
		frappe.throw(_("Only submitted Stock Take Entries can create a Stock Reconciliation."))
	...
	for row in doc.items:
		if not row.item_code:
			frappe.throw(
				_("Row #{0}: Item Code is required to create a Stock Reconciliation.").format(
					row.idx
				)
			)
		...
		if not warehouse:
			frappe.throw(
				_("Row #{0}: Please set a Warehouse on the row or Default Warehouse on the entry.")
				.format(row.idx)
			)
